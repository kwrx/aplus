...
long sys_clone (int (*fn)(void*), void __user * stack, unsigned long flags, void* arg) {
    ...
    if(unlikely(!ptr_check(stack, R_OK | W_OK | X_OK)))
        return -EFAULT;


    task_t* child = (task_t*) kcalloc(sizeof(task_t), 1, GFP_KERNEL);
    
    child->tid = sched_nextpid();
    child->status = TASK_STATUS_READY;
    

    __lock_irq(&current_task->lock, {
        ...
        //arch_task_set_context(child->frame.context, )
