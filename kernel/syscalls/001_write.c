...
/* `seq` is sampled before the attempt rather than after it. The inode's change counter is
   bumped by whoever makes the descriptor ready, and a snapshot taken once the attempt has
   already failed has that bump in it -- so the sleep waits for a wakeup that has been and
   gone, and nothing will ever move the word again. On one CPU the window between the two is
   a preemption wide; on four the other end really is running at the same time. */
