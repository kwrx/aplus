...
/* Every walk of a run queue is held under that CPU's sched_lock: it is the lock
   ...
   the cursor stands on from being handed back to the heap. The mask is copied out to user
   memory only once that lock is down, since writing to it can fault and a run queue is not
   something to be holding when it does. */
