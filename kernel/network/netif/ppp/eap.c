 ...
 */
static const u_char wkmodulus[] = {};
    #endif
...
static void eap_figure_next_state(ppp_pcb* pcb, int status) {
    ...
    switch (pcb->eap.es_server.ea_state) {
        ...
        case eapIdentify:
            ...
            if (pcb->eap.es_server.ea_peerlen > SRP_PSEUDO_LEN && strncmp(pcb->eap.es_server.ea_peer, SRP_PSEUDO_ID, SRP_PSEUDO_LEN) == 0 && (pcb->eap.es_server.ea_peerlen - SRP_PSEUDO_LEN) * 3 / 4 < sizeof(secbuf)) {
                BZERO(&bs, sizeof(bs));
...
static void eap_success(ppp_pcb* pcb, u_char* inp, int id, int len) {
    ...
    if (pcb->eap.es_client.ea_state != eapOpen && !eap_client_active(pcb)) {
        ppp_dbglog("EAP unexpected success message in state %s (%d)", eap_state_name(pcb->eap.es_client.ea_state), pcb->eap.es_client.ea_state);
        return;
    }
...
static void eap_failure(ppp_pcb* pcb, u_char* inp, int id, int len) {
    ...
    if (!eap_client_active(pcb)) {
        ppp_dbglog("EAP unexpected failure message in state %s (%d)", eap_state_name(pcb->eap.es_client.ea_state), pcb->eap.es_client.ea_state);
    }
...
static const char* const eap_typenames[] = {"Identity", "Notification", "Nak", "MD5-Challenge", "OTP", "Generic-Token", NULL, NULL, "RSA", "DSS", "KEA", "KEA-Validate", "TLS", "Defender", "Windows 2000", "Arcot", "Cisco", "Nokia", "SRP"};
