...
__percpu void apic_enable(void) {
    ...
    x86_wrmsr(X86_APIC_BASE_MSR, X86_APIC_BASE_ADDR | X86_APIC_MSR_EN | (x2apic ? X86_APIC_MSR_EXTD : 0) | (msr & 0x3FF));
...
void apic_init(void) {
    ...
    if (X86_APIC_BASE_ADDR < ((core->memory.phys_upper + core->memory.phys_lower) * 1024)) {
        ...
        pmm_claim_area(X86_APIC_BASE_ADDR, PML1_PAGESIZE);
    }


    if (!x2apic) {

        arch_vmm_map(&core->bsp.address_space, X86_APIC_BASE_ADDR, X86_APIC_BASE_ADDR, PML1_PAGESIZE,
