    ...
    if (pcur)
    {
        ...
        if (pcur->dirty)
        {
            if (fs->disk_io.write_media)
            {
                ...
                if (!fs->disk_io.write_media(fs, pcur->address, pcur->sector, sectors))
                    return 0;
            }
...
static struct fat_buffer *fatfs_fat_read_sector(struct fatfs *fs, uint32 sector)
{
    ...
    // Read next sector
    if (!fs->disk_io.read_media(fs, pcur->address, pcur->sector, FAT_BUFFER_SECTORS))
    {
...
void fatfs_set_fs_info_next_free_cluster(struct fatfs *fs, uint32 newValue)
{
    ...
    else
    {
        ...
        if (fs->disk_io.write_media)
            fs->disk_io.write_media(fs, pbuf->address, pbuf->sector, 1);
