...
static int fatfs_erase_sectors(struct fatfs *fs, uint32 lba, int count)
{
    ...
    for (i=0;i<count;i++)
        if (!fs->disk_io.write_media(fs, lba + i, fs->currentsector.sector, 1))
            return 0;
...
static int fatfs_create_boot_sector(struct fatfs *fs, uint32 boot_sector_lba, uint32 vol_sectors, const char *name, int is_fat32)
{
    ...
    if (fs->disk_io.write_media(fs, boot_sector_lba, fs->currentsector.sector, 1))
        return 1;
    else
        return 0;
}
...
static int fatfs_create_fsinfo_sector(struct fatfs *fs, uint32 sector_lba)
{
    ...
    if (fs->disk_io.write_media(fs, sector_lba, fs->currentsector.sector, 1))
        return 1;
    else
        return 0;
}
...
static int fatfs_erase_fat(struct fatfs *fs, int is_fat32)
{
    ...
    if (!fs->disk_io.write_media(fs, fs->fat_begin_lba + 0, fs->currentsector.sector, 1))
        return 0;

    // Zero remaining FAT sectors
    memset(fs->currentsector.sector, 0, FAT_SECTOR_SIZE);
    for (i=1;i<fs->fat_sectors*fs->num_of_fats;i++)
        if (!fs->disk_io.write_media(fs, fs->fat_begin_lba + i, fs->currentsector.sector, 1))
            return 0;
