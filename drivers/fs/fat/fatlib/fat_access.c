...
int fatfs_init(struct fatfs *fs)
{
    ...
    if (!fs->disk_io.read_media(fs, 0, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;
    ...
    if (!fs->disk_io.read_media(fs, fs->lba_begin, fs->currentsector.sector, 1))
        return FAT_INIT_MEDIA_ACCESS_ERROR;
...
{
    return fs->disk_io.read_media(fs, lba, target, count);
}
//-----------------------------------------------------------------------------
// fatfs_sector_write:
//-----------------------------------------------------------------------------
...
{
    return fs->disk_io.write_media(fs, lba, target, count);
}
//-----------------------------------------------------------------------------
...
int fatfs_sector_reader(struct fatfs *fs, uint32 start_cluster, uint32 offset, uint8 *target)
{
    ...
    if (target)
        return fs->disk_io.read_media(fs, lba, target, 1);
    // Else read sector if not already loaded
    else if (lba != fs->currentsector.address)
    {
        fs->currentsector.address = lba;
        return fs->disk_io.read_media(fs, fs->currentsector.address, fs->currentsector.sector, 1);
    }
    else
        return 1;
}
...
int fatfs_write_sector(struct fatfs *fs, uint32 cluster, uint32 sector, uint8 *target)
{
    ...
    if (fs->fat_type == FAT_TYPE_16 && cluster == 0)
    {
        ...
        {
            // Write to disk
            return fs->disk_io.write_media(fs, lba, target, 1);
        }
        else
        {
            // Calculate write address
            ...
            // Write to disk
            return fs->disk_io.write_media(fs, fs->currentsector.address, fs->currentsector.sector, 1);
        }
    }
    // FAT16/32 Other
    else
