...
__fastcall
__optimize(fast)
static void console_output_render_16(context_t* cx, uint16_t sr, uint16_t er, uint16_t sc, uint16_t ec) {
    long sy = (sr * ROW);
    long ey = (er * ROW);
    long sx = (sc * COL);
    long ex = (ec * COL);
    ...
    for(; sy < ey; sy++)
        memcpy (
            (void*) ((uintptr_t) cx->screen.framebuffer + (sy * cx->screen.stride) + (sx << 1)),
