...
__noreturn __nosanitize("undefined") void __ubsan_handle_out_of_bounds(struct out_of_bounds_data* data, uintptr_t index) {
    ...
    kpanicf("PANIC! UBSAN: index %ld of type %s out of bounds for object type %s on %s:%d:%d\n", index, data->index_type->name, data->array_type->name, data->location.file, data->location.line, data->location.column);
}
