...
__noreturn __nosanitize("undefined") void __ubsan_handle_invalid_builtin(struct invalid_builtin_data* data) {
    ...
    kpanicf("PANIC! UBSAN: invalid builtin of kind %s on %s:%d:%d\n", __kinds[data->kind], data->location.file, data->location.line, data->location.column);
}
