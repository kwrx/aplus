...
#define inv_mix_col(x, f2, f4, f8, f9) ((f2) = mul2(x, f2), (f4) = mul2(f2, f4), (f8) = mul2(f4, f8), (f9) = (x) ^ (f8), (f8) = ((f2) ^ (f4) ^ (f8)), (f2) ^= (f9), (f4) ^= (f9), (f8) ^= rot3(f2), (f8) ^= rot2(f4), (f8) ^ rot1(f9))
...
static const uint8_t aes_sbox[256] = {
    0x63, 0x7C, 0x77, 0x7B, 0xF2, 0x6B, 0x6F, 0xC5, 0x30, 0x01, 0x67, 0x2B, 0xFE, 0xD7, 0xAB, 0x76, 0xCA, 0x82, 0xC9, 0x7D, 0xFA, 0x59, 0x47, 0xF0, 0xAD, 0xD4, 0xA2, 0xAF, 0x9C, 0xA4, 0x72, 0xC0, 0xB7, 0xFD, 0x93, 0x26, 0x36,
 ...
 * AES is-box
 */
static const uint8_t aes_isbox[256] = {0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb, 0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb,
...
void aes_encrypt(aes_t* ctx, const uint8_t* msg, uint8_t* out, size_t length) {
    ...
    for (length -= AES_BLOCKSIZE; length >= 0; length -= AES_BLOCKSIZE) {
        ...
        for (i = 0; i < 4; i++)
            tin[i] = __builtin_bswap32(msg_32[i]) ^ tout[i];

        AES_encrypt(ctx, tin);

        for (i = 0; i < 4; i++) {
            tout[i]   = tin[i];
            out_32[i] = __builtin_bswap32(tout[i]);
        }
...
void aes_decrypt(aes_t* ctx, const uint8_t* msg, uint8_t* out, size_t length) {
    ...
    for (length -= 16; length >= 0; length -= 16) {
        ...
        for (i = 0; i < 4; i++) {
            tin[i]  = __builtin_bswap32(msg_32[i]);
            data[i] = tin[i];
        }
